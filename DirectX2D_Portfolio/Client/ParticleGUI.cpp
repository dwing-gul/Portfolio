...
void ParticleGUI::update()
{
	if (Start())
	{
		Particle* particle = GetTarget()->GetParticle();

		vector<Ptr<Texture>>& vectorTexture = particle->GetVectorTexture();
		...
		if(!vectorTexture.empty())
			GetResourceName(vectorTexture[textureIndex].Get(), textureName);
		...
		if (ImGui::Button("Select##Texture_Particle"))
		{
			...
			GetTextureName(vectorName);
			listGUI->SetList(vectorName);
			listGUI->SetDoubleClickedCallBack(this, (GUI_CALLBACK)&ParticleGUI::SetTexture);
			listGUI->Activate();
		}
...
void ParticleGUI::SetTexture(DWORD_PTR _data1, DWORD_PTR _data2)
{
	...
	map<wstring, Res*> mapTexture = ResourceManager::GetInst()->GetRes(RESOURCE_TYPE::TEXTURE);

	map<wstring, Res*>::iterator iter = mapTexture.find(key);
	if (iter == mapTexture.end())
		return;

	vector<Ptr<Texture>>& vectorTexture = particle->GetVectorTexture();
	vectorTexture.push_back(ResourceManager::GetInst()->Load<Texture>(iter->first, iter->second->GetRelativePath()));
...
void ParticleGUI::GetTextureName(vector<wstring>& _vectorName)
{
	map<wstring, Res*> mapTexture = ResourceManager::GetInst()->GetRes(RESOURCE_TYPE::TEXTURE);

	map<wstring, Res*>::iterator iter = mapTexture.begin();
